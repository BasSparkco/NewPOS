...
- [~] T6 — Isolation, migration and resilience release tests. (Closed the ManageStores upgrade path, per-request store-access revocation enforcement, retryable-vs-permanent sync conflict classification, and confirmed invoice/inventory/stock-ledger rollback atomicity this session, plus cross-tenant cash-session-close and stale-token-after-revocation rows; still not a completed gate — the offline last-unit-sale reconciliation policy, a web stock-adjustment/device-reconnect test, and the PostgreSQL migration/concurrency rows remain open — see STATUS.md.)
...
*Last updated 2026-09-22 (later same day): closed T6's four explicitly-named gaps — an always-applied migration upgrades every existing tenant's Admin role to `ManageStores` without expanding any custom role; `POS.Web` cookie auth now re-validates store access on every request, not just at switch time; sync push conflict handling now distinguishes retryable concurrency/duplicate conflicts (409) from permanent database validation failures (422); and a new test confirms a failed concurrent push rolls back its invoice, inventory and stock-ledger writes together. Two more matrix rows closed opportunistically (stale token after revocation, cross-tenant cash-session close). PostgreSQL live rehearsal and T7 remain explicitly pending — re-verified, not assumed, that no Docker/PostgreSQL is available in this environment. See [STATUS.md](STATUS.md) Stage 4T for the authoritative, detailed record, including the two T6 rows that remain open pending a product decision rather than a bug fix (offline last-unit-sale reconciliation; a web stock-adjustment surface that doesn't exist yet). Earlier history: Stages 0–4 complete; T0–T5 substantially landed across several prior sessions (device/register/cash-session work, sync isolation fixes, offline-authorization expiry, tenant-aware store administration) — see STATUS.md's dated entries for the full trail.
