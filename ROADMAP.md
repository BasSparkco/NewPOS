...
- [~] T6 — Isolation, migration and resilience release tests. (Materially more matrix-row coverage this session — concurrent double-push and cross-store/cross-tenant store-access rows are now tested; still not a completed gate — see STATUS.md.)
...
*Last updated 2026-09-22: fixed a real concurrent-double-push gap (`Invoice.SyncVersion` is now an EF concurrency token, and `/api/sync/invoices/push` fails a racing batch closed instead of leaking an unhandled 500 or silently double-recording a stock movement) and implemented T5 — tenant-aware Store CRUD, `UserStoreAccess`-backed multi-store grants, and an authorized active-store selector in `POS.Web`, all enforced server-side. See [STATUS.md](STATUS.md) Stage 4T for the authoritative, detailed record of exactly what landed, what's still open, and the next milestone (the live PostgreSQL rehearsal, T6 completion, then T7 remain explicit pre-production gates). Earlier history: Stages 0–4 complete; T0–T4.5 substantially landed across several prior sessions (device/register/cash-session work, sync isolation fixes, offline-authorization expiry) — see STATUS.md's dated entries for the full trail.
