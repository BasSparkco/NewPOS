...
- [~] T7 — Recovery rehearsal and controlled pilot. (Isolated staging deployment stood up, internally verified against live PostgreSQL, and now externally reachable over trusted HTTPS — see STATUS.md. The WPF manual rehearsal (`docs/STAGING_WPF_REHEARSAL.md`) is ready to run; backup/restore rehearsal and a real pilot business remain unstarted.)
...
7. **Receipt printer selection setting (added 2026-09-23, found during the staging WPF rehearsal):** `EscPosReceiptPrinter` (`POS.Wpf`) always prints to whatever Windows currently has set as the *system-wide default* printer, with no app-level override. During the rehearsal this caused a real, reproduced ~6s stall on sale completion (the Win32 print-spooler call blocking the UI thread while trying to reach an unreachable default printer) — the blocking-UI-thread part is already fixed (printing now runs on a background `Task.Run`, never delays the success message), but the *which printer* ambiguity is not. Proposed: a "Receipt printer" dropdown in Settings, enumerating installed Windows printers (`LocalPrintServer.GetPrintQueues()`), persisted as a store setting, with `EscPosReceiptPrinter` targeting that specific queue instead of the OS default (falling back to the OS default only if unset). Deliberately **not** proposing app-level printer-driver/brand management (Zebra profiles, protocol selection, etc.) — Windows' own driver layer already handles that for any ESC/POS-capable receipt printer (including Zebra's receipt-class printers, distinct from their ZPL label printers), and reimplementing it would be ongoing per-brand maintenance for something the OS already solves.
8. **A device/till's identity should not be silently keyed off the Windows computer name (added 2026-09-25, found during the staging WPF rehearsal):** `SaleService.GetOrCreateCurrentDeviceAsync` matches an existing local `Device` row by `Name == Environment.MachineName` — the rehearsal laptop was renamed mid-session, which would (had a sale been attempted after the rename) silently create a *second*, unrelated Device+Register instead of recognizing the same physical till, fragmenting its sales/cash-session history across two identities with no warning to the cashier or admin. `Register` already exists precisely to give a till a stable identity that survives a hardware swap (tenant.md §2.3a), but nothing today gives it a *name* independent of the underlying machine's OS-level name. Proposed: an editable "This till's name" field in Settings (WPF, and eventually Web), stored once at first provisioning and never re-derived from `Environment.MachineName` again, so renaming the Windows PC — or eventually replacing the hardware entirely, which `Register` already supports — never fragments a till's identity. A smaller, cheaper first step: at minimum, warn (don't silently proceed) if `Environment.MachineName` no longer matches any known local `Device.Name` but a `Register` already exists locally, so a rename is caught before it creates a orphaned duplicate. Related, same investigation: consider whether *any other* Settings the app should let an operator confirm/override at setup time (store display name shown on receipts, register number, timezone if reports ever need one) are currently only inferable rather than explicitly configurable — not scoped out further here, flagged for a future audit pass rather than guessed at now.
...
**⏸ Session paused mid-rehearsal (2026-09-25) — see `start.md`'s "Resume here" section for exact next steps.** Two new backlog items were added to Operational Extensions during it: a receipt-printer selection setting, and making device/till identity independent of the Windows computer name (both found live — see STATUS.md's dated rehearsal entry).
